...
class CountyScraper:
    ...
    def GetCountyList(self):
        ...
        if ret.status_code == 200:
            trs = Selector(text=ret.text).xpath('//table[@class="table table-condensed table-striped agency-list"][2]/tbody/tr').extract()

            counties = []
            for tr in trs:
                county = {
                    'county_name': Selector(text=tr).xpath('//td[1]/a/text()').extract()[0],
            ...
            print('failed to get county list')
            return None
    ...
    def Start(self):
        ...
        for county in counties:
            ...
            # get csv file list
            print('getting csv file list about %s ...' % (county_name))
            csv_list = self.GetCsvList(county_url)
