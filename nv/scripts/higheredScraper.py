...
#--------------------define variables-------------------
FOLDER_PATH = 'highered/'
OUTPUT_FILE = 'highered.csv'
#-------------------------------------------------------
...
class HigheredScraper:
    ...
    def GetHigheredList(self):
        ...
        if ret.status_code == 200:
            trs = Selector(text=ret.text).xpath('//table[@class="table table-condensed table-striped agency-list"][3]/tbody/tr').extract()

            highered_list = []
            for tr in trs:
                highered = {
                    'highered_name': Selector(text=tr).xpath('//td[1]/a/text()').extract()[0],
                    'url': self.base_url + Selector(text=tr).xpath('//td[1]/a/@href').extract()[0]
                }
                highered_list.append(highered)

            print(highered_list)
            return highered_list
        else:
            print('failed to get highered list')
            return None
    ...
    def Start(self):
        # get highered list
        print('getting highered list ...')
        highered_list = self.GetHigheredList()

        for highered in highered_list:
            highered_name = highered['highered_name']
            highered_url = highered['url']

            # get csv file list
            print('getting csv file list for %s ...' % (highered_name))
            csv_list = self.GetCsvList(highered_url)
